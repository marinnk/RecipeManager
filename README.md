...
このアプリを通して、以下のことができるようになる。

- **レシピの登録**: YouTube動画やクックパッドなどのレシピサイトのURLを入力すると、タイトル・サムネイル画像を自動取得して保存できる（自動取得に失敗した場合は、タイトルを手動入力し、画像ファイルをアップロードして登録できる）
- **メモ・タグ付け**: 登録時に自由なメモと複数のタグを付けられる
- **一覧表示**: 登録済みレシピをカードで一覧表示できる
- **検索・絞り込み**: タイトルのキーワード・タグでレシピを検索・絞り込みできる
- **詳細表示**: メモ・タグとともにレシピを表示し、元のYouTube動画／レシピサイトへのリンクから遷移できる
- **編集・削除**: 登録済みレシピのタイトル・サムネイル・メモ・タグを編集、または削除できる
