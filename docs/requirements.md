...
| # | 機能 | 内容 |
|---|------|------|
| 1 | レシピ登録 | URLを入力すると、タイトル・サムネイル画像を自動取得する。自動取得に失敗した場合はタイトルを手動入力し、画像ファイルをアップロードして添付できる。あわせてメモ（自由テキスト）とタグ（自由入力、複数可）を登録できる |
...
YouTubeの動画URLやクックパッド等レシピサイトのURLから、タイトル・サムネイル画像を自動で取得し、登録の手間を減らす。取得に失敗した場合は、ユーザーが手動でタイトルを入力し、画像ファイルをアップロードできるようにする。具体的な技術方式は [基本設計書](./basic-design.md) を参照。
...
- [x] YouTubeのURLを貼ると、タイトル・サムネイルが自動表示される
- [x] クックパッド等レシピサイトのURLを貼ると、タイトル・サムネイルが自動表示される
- [x] 自動取得に失敗した場合でも、手動でタイトルを入力し、画像ファイルをアップロードして登録できる
- [x] メモとタグ（複数）を付けてレシピを登録できる
- [x] 登録したレシピが一覧画面にカード表示される
- [x] タグ名・キーワードでレシピを検索・絞り込みできる
- [x] レシピ詳細画面から元のURLに遷移できる
- [x] 登録済みレシピの編集・削除ができる
- [x] AWS上にデプロイされ、ブラウザから継続的にアクセスできる
- [x] AWSの無料枠を超える課金が発生していない
