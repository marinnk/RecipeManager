...
| レシピ登録画面 | URL入力欄、自動取得プレビュー（タイトル・サムネイル、手動修正可）、メモ・タグ入力欄、サムネイル画像のファイルアップロード（自動取得に失敗した場合の代替手段） |
...
| thumbnail_url | varchar | サムネイル画像のアクセス用パス。外部URL（YouTube等）または、ユーザーがアップロードした画像へのパス（`/api/uploads/xxxx.jpg`）のいずれかを保持する |
| memo | text | ユーザーの自由メモ |
...
いずれも取得に失敗した場合は、ユーザーが手動でタイトルを入力し、画像ファイルをアップロードして添付できるようフォールバックする（画像URLの手動入力は求めない）。アップロードされた画像はEC2のローカルディスクに保存する（詳細は「7. AWS構成方針」を参照）。
...
- 具体的なVPC・サブネット構成は「7.1 VPC・ネットワーク構成」、セキュリティグループは「7.2 セキュリティグループ」を参照
- **アップロード画像の保存先**: ユーザーがアップロードしたサムネイル画像は、S3等のオブジェクトストレージは使わずEC2のローカルディスクに保存する。追加のAWSインフラが不要になる一方、コンテナを再デプロイするとディスク上の画像は失われる。本アプリは個人利用の練習用アプリであり、画像が消えても実害が無いためこの制約を許容する（本番運用が必要になった場合はS3への移行を検討する）

### 7.1 VPC・ネットワーク構成

- **リージョン**: ap-northeast-1（東京）
- **VPC**: `10.0.0.0/16` を1つ作成する
- **サブネット**: パブリックサブネット `10.0.1.0/24`（ap-northeast-1a）を作成し、EC2はここへ配置する。「9. 非機能要件への対応方針」の通り単一AZ構成とする方針に変更はない。ただし、RDSの「DBサブネットグループ」はAWS側の仕様上、異なる2つ以上のAZにまたがるサブネットを登録する必要があるため、`10.0.2.0/24`（ap-northeast-1c）をRDSのサブネットグループ登録専用にもう1つ用意する（RDSインスタンス自体は引き続きMulti-AZ化はせず、単一AZで稼働する）
- **インターネットゲートウェイ（IGW）**: VPCにアタッチし、パブリックサブネットのルートテーブルに `0.0.0.0/0 → IGW` を設定する（EC2・RDSともに、外部との通信はこのIGW経由になる）
- **Elastic IP**: EC2に1つ割り当てる。割り当てないと、EC2の再起動のたびにパブリックIPが変わり、CloudFrontのオリジン設定（EC2のIP／ドメインを指す設定）が壊れてしまうため、固定IPを用意する（EC2にアタッチして起動中は無料）

```mermaid
flowchart TB
    IGW["インターネットゲートウェイ"]
    subgraph VPC["VPC 10.0.0.0/16"]
        subgraph Subnet["パブリックサブネット 10.0.1.0/24（ap-northeast-1a）"]
            EC2["EC2（Elastic IP付き）"]
            RDS[("RDS (MySQL)")]
        end
    end
    CF["CloudFront"] --> IGW --> EC2
    EC2 -- "3306番のみ許可" --> RDS
```
...
| POST | `/api/images` | サムネイル画像ファイルをアップロードし、保存先パスを取得する | レシピ登録（画像添付） |

`GET /api/uploads/{filename}` は上記とは別に、アップロード済みの画像ファイルそのものを配信する静的リソース配信であり、通常のJSON REST APIではない（Spring MVCの静的リソースハンドラで配信する）。
...
**POST /api/images（`multipart/form-data`、フィールド名`file`）**
```sh
curl -F "file=@photo.jpg" http://localhost:8080/api/images
```
成功時レスポンス（`201 Created`）:
```json
{ "url": "/api/uploads/3f2504e0-4f89-11e9-8646-d663bd873d93.jpg" }
```
許可する形式はjpeg・png・webp、サイズ上限は8MB。画像は登録前に単体でアップロードし、返ってきた`url`をそのまま`POST /api/recipes`の`thumbnailUrl`に渡す（2段階方式）。
